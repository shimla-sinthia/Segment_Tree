...
void build_seg_tree(ll sii, ll sss, ll see)
{
   if(sss == see)
   {
      seg[sii] = mp(arr[see], 1); //arr[see];
      return;
   }
   ll mid = (sss + see) / 2;
   build_seg_tree(2*sii , sss , mid);
   build_seg_tree(2*sii+1 , mid+1 , see);
   if(seg[2*sii].ff < seg[2*sii + 1].ff)
   {
     seg[sii].ff = seg[2 * sii].ff;
     seg[sii].ss = seg[2 * sii].ss;
   }
   else if(seg[2 * sii + 1].ff < seg[2 * sii].ff)
   {
     seg[sii].ff = seg[2 * sii + 1].ff;
     seg[sii].ss = seg[2 * sii + 1].ss;
   }
   else
   {
     seg[sii].ff = seg[2 * sii].ff;
   ...
   // seg[sii].ff = min(seg[2*sii].ff , seg[2*sii+1].ff);
   // seg[sii].ss = seg[2*sii].ss + seg[2*sii+1].ss;
}
pair<ll, ll> query(ll sii, ll sss, ll see, ll qs, ll qe)
{
   if(qs > see || qe < sss)
   return mp(1e12, -1);
   if(sss >= qs && see <= qe)
   return seg[sii];
   ll mid = (sss + see) / 2;
   pair<ll, ll> l = query(2*sii , sss , mid , qs , qe);
   pair<ll, ll> r = query(2*sii+1 , mid+1 , see , qs , qe);
   pair<ll, ll>q;
   if(l.ff < r.ff)
   {
    q = l;
